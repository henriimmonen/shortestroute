...
class TestRoute(unittest.TestCase):
    def setUp(self):
        self.kartta = ['...', '.@@', '...']
        self.kartta_hyppysolmu_horisontaalinen = ['..@..', '..@..', '.@...', '.@...']
        self.kartta_hyppysolmu_vertikaalinen = ['..@', '@..', '...']
    ...
    def test_etsi_horisontaalisesti_tunnistaa_hyppysolmun_ylaoikea(self):
        verkko = self.apufunktiot.kaarilista(self.kartta_hyppysolmu_horisontaalinen)
        jps_hyppysolmu = JumpPointSearch((0, 0), (3, 4), verkko, self.kartta_hyppysolmu_horisontaalinen)
        self.assertEqual((2, 2), jps_hyppysolmu.etsi_horisontaalisesti((2, 2), 0, -1))

    def test_etsi_horisontaalisesti_tunnistaa_hyppysolmun_alaoikea(self):
        verkko = self.apufunktiot.kaarilista(self.kartta_hyppysolmu_horisontaalinen)
        jps_hyppysolmu = JumpPointSearch((0, 0), (3, 4), verkko, self.kartta_hyppysolmu_horisontaalinen)
        self.assertEqual((1, 1), jps_hyppysolmu.etsi_horisontaalisesti((1, 1), 0, 1))

    def test_etsi_vertikaalisesti_tunnistaa_hyppysolmun_alaoikea(self):
        verkko = self.apufunktiot.kaarilista(self.kartta_hyppysolmu_vertikaalinen)
        jps_hyppysolmu = JumpPointSearch((0, 0), (2, 2), verkko, self.kartta_hyppysolmu_vertikaalinen)
        self.assertEqual((0, 1), jps_hyppysolmu.etsi_vertikaalisesti((0, 1), 1, 0))

    def test_etsi_vertikaalisesti_tunnistaa_hyppysolmun_alavasen(self):
        verkko = self.apufunktiot.kaarilista(self.kartta_hyppysolmu_vertikaalinen)
        jps_hyppysolmu = JumpPointSearch((0, 0), (2, 2), verkko, self.kartta_hyppysolmu_vertikaalinen)
        self.assertEqual((1, 1), jps_hyppysolmu.etsi_vertikaalisesti((1, 1), 1, 0))
